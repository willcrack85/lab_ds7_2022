...
class noticia extends modeloCredencialesBD
{
    ...
    //// Nuevo metodo agregado y consumido por lab111.php
    public function mostrar_paginacion_tabla($inicio, $total)
    {
        $instruccion = "CALL sp_listar_noticia_paginada('" . $inicio . "','" . $total . "')";
        $consulta = $this->_db->query($instruccion);
        $resultado = $consulta->fetch_all(MYSQLI_ASSOC);
        if ($resultado) {
            return $resultado;
