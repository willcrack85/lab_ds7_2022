...
<body class="d-flex flex-column h-100">
    ...
    <main role="main" class="flex-shrink-0">
        <div class="jumbotron">
            <div class="container">
                <h1 class="display-3">DS7 - Laboratorio 11.1</h1>
            ...
            <div class="row m-1">
                <div class='jumbotron-wc1 border border-white col-md-12'>
                    <div class="shadow-lg p-3 mb-2 m-2 bg-white text-primary rounded">
                        <h2 class="mt-1 text-center">CONSULTA DE NOTICIAS EN TABLA</h2>
                        <?php 
                            if(!$_GET) {
                            ...
                            $obj_noticia = new noticia();
                            $noticias_paginada = $obj_noticia-> mostrar_paginacion_tabla($inicio, PAGE);
                        ...
                        <form name='formFiltro' method='POST' action='lab111.php'>
                            <div class='input-group mb-1'>
                                <nav aria-label="MiPaginacionDS7">
                                    <ul class="pagination">
                                        <li class="page-item <?php echo($_GET['page'] <= 1 ? 'disabled' : '')?>" aria-current="page">
                                            <a class="page-link" href="<?php echo($_SERVER['PHP_SELF']."?page=".($_GET['page'] - 1))?>">
                                                Anterior
                                            </a>
                                        </li>                  
                                        <?php for($i=0; $i < $paginas; $i++): ?>
                                        <li class="page-item <?php echo($_GET['page'] == ($i + 1) ? 'active' : '')?>" aria-current="page">
                                        ...
                                        <li class="page-item <?php echo($_GET['page'] >= $paginas ? 'disabled' : '')?>">
                                            <a class="page-link" href="<?php echo($_SERVER['PHP_SELF']."?page=".($_GET['page'] + 1))?>">Siguiente</a>
                                        </li>
                                    </ul>
                                </nav>
                                <label class="col-sm-7 col-form-label"><?php print("Mostrar noticias ".($inicio + 1)." a ".($inicio + $filas_paginado)." de un total de $nfilas.")?></label>
                            </div>
                        </form>
                    </div>
